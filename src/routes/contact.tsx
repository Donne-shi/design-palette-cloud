...
function ContactPage() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    setSending(true);
    setTimeout(() => { setSending(false); toast.success(t("已收到您的留言，我们会尽快回复。","Message received — we'll reply soon.")); (e.target as HTMLFormElement).reset(); }, 600);
  };
  return (
    <SiteShell>
      <PageHero
      ...
      <section className="container-prose py-20 grid md:grid-cols-12 gap-12">
        <div className="md:col-span-5 space-y-8">
          <div>
            ...
            <p className="serif text-xl flex items-center gap-2"><Mail className="h-4 w-4 text-accent" /> hello@mbi.example</p>
          </div>
          <div>
            <p className="eyebrow mb-2">Editorial · 投稿</p>
            <p className="serif text-xl flex items-center gap-2"><Mail className="h-4 w-4 text-accent" /> editor@mbi.example</p>
          </div>
          <div>
        ...
        <form onSubmit={submit} className="md:col-span-7 space-y-5">
          <div className="grid sm:grid-cols-2 gap-5">
            <label className="block">
              ...
              <input required className="mt-2 w-full bg-background border border-input px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-accent" />
            </label>
            <label className="block">
              <span className="text-xs uppercase tracking-widest text-stone-warm">Email</span>
              <input required type="email" className="mt-2 w-full bg-background border border-input px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-accent" />
            </label>
          </div>
          <label className="block">
            <span className="text-xs uppercase tracking-widest text-stone-warm">{t("主题","Subject")}</span>
            <input className="mt-2 w-full bg-background border border-input px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-accent" />
          </label>
          <label className="block">
            <span className="text-xs uppercase tracking-widest text-stone-warm">{t("留言","Message")}</span>
            <textarea required rows={6} className="mt-2 w-full bg-background border border-input px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-accent resize-none" />
          </label>
